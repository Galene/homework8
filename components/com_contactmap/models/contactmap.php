...
class ContactMapsModelContactMap extends JModel
{
    ...
    function _getQuery()
    {
        ...
        $select = 'a.*, us.email as user_email, cc.access as category_access, cc.title as category_name, cc.description as cat_description,'
        . ' CASE WHEN CHAR_LENGTH(a.alias) THEN CONCAT_WS(\':\', a.id, a.alias) ELSE a.id END as slug, '
        . ' CASE WHEN CHAR_LENGTH(cc.alias) THEN CONCAT_WS(\':\', cc.id, cc.alias) ELSE cc.id END AS catslug ';
        $from   = '#__contact_details AS a';

        $joins[] = 'INNER JOIN #__categories AS cc on cc.id = a.catid';
        $joins[] = 'LEFT OUTER JOIN #__users AS us on us.id = a.user_id';
...
//Création des marqueurs
	...
	for (var i = 0; i < donnees.length; i++) {
		...
		attachinfowindow(marker_map[i],place, i);
	}
	'
	.$centrageauto
	.$Zauto
